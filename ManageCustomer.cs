    ...
    public partial class ManageCustomer : Form
    {
            ...
            private void DeleteCustomer(int customerId)
            {
                ...
                if (connection != null)
                {
                    ...
                    string deleteSaleDetailQuery = "DELETE SaleDetail WHERE SaleDetailID IN " +
                                                    "(SELECT SaleID FROM Sale WHERE CustomerID = @customerId)";
                    SqlCommand command = new SqlCommand(deleteSaleDetailQuery, connection);
                    command.Parameters.AddWithValue("customerId", customerId);
                    command.ExecuteNonQuery();

                    string deleteSaleQuery = "DELETE FROM Sale WHERE CustomerID = @customerId";
                    command = new SqlCommand(deleteSaleQuery, connection);
                    command.Parameters.AddWithValue("customerId", customerId);
                    command.ExecuteNonQuery();
